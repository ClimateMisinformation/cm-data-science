...
######################## NORMALISED BOW EMBEDDINGS ################################



def norm_bow_vectorizer(df):

    print("Initialising normalised BOW model")
    ...
    print("Constructing normalised BOW vectors")

    embedding_df = vector_constructor(model, df)

    embedding_df['classes'] = df['labels'].to_list()
...
def vector_constructor(model, df):

    docs = df['clean_text'].to_list()

    print("Number of statements", len(docs))
